...
type ProcessMessageContext = {
	...
	logger?: ILogger
	options: RequestInit
	signalRepository: SignalRepositoryWithJIDStore
...
export const cleanMessage = (message: WAMessage, meId: string, _meLid?: string) => {
	// ensure remoteJid and participant doesn't have device or agent in it
	// JID-first: normalize hosted PN or LID remoteJid to plain s.whatsapp.net JID
	if (isHostedPnUser(message.key.remoteJid!) || isHostedLidUser(message.key.remoteJid!)) {
		message.key.remoteJid = jidEncode(
			jidDecode(message.key?.remoteJid!)?.user!,
			's.whatsapp.net' // JID-first: always normalize to phone JID
		)
	} else {
		message.key.remoteJid = jidNormalizedUser(message.key.remoteJid!)
	}

	// JID-first: normalize participant to phone JID
	...
	function normaliseKey(msgKey: WAMessageKey) {
		...
		if (!message.key.fromMe) {
			...
			// JID-first: match self by phone JID only
	...
	if (protocolMsg) {
		switch (protocolMsg.type) {
			case proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
				...
				if (process) {
					...
					if (data.lidPnMappings?.length) {
						logger?.debug({ count: data.lidPnMappings.length }, 'processing LID→JID mappings from history sync')
						// Use storeJIDPNMappings — history data uses { jid, pn } format
						await signalRepository.jidMapping
							.storeJIDPNMappings(data.lidPnMappings)
							.catch(err => logger?.warn({ err }, 'failed to store LID→JID mappings from history sync'))
					}
			...
			case proto.Message.ProtocolMessage.Type.LID_MIGRATION_MAPPING_SYNC:
				...
				// JID-first: store LID→JID mappings so incoming LID msgs resolve correctly
				await signalRepository.jidMapping.storeLIDPNMappings(pairs)
		}
		...
		if (eventMsg) {
			try {
				...
				// JID-first: resolve any LID to JID
				const eventCreatorPn = isLidUser(eventCreatorKey)
					? await signalRepository.jidMapping.resolveToJID(eventCreatorKey)
					: eventCreatorKey
		...
		const emitGroupRequestJoin = (participant: JIDMapping, action: RequestJoinAction, method: RequestJoinMethod) => {
			ev.emit('group.join-request', {
				id: jid,
				author: message.key.participant!,
				authorPn: message.key.participantAlt!,
				participant: participant.jid, // JID-first: was participant.lid
		...
		switch (message.messageStubType) {
			...
			case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD: // TODO: Add other events
				const participant = JSON.parse(message.messageStubParameters?.[0]) as JIDMapping
