...
export type IdentityChangeContext = {
	...
	/** @deprecated JID-first: identity always matched by meId */
...
export async function handleIdentityChange(
	...
	// JID-first: match self by phone number JID only
	const isSelfPrimary = ctx.meId && areJidsSameUser(from, ctx.meId)
	if (isSelfPrimary) {
