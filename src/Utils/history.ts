...
import type { Chat, Contact, JIDMapping, WAMessage } from '../Types'
...
export const processHistoryMessage = (item: proto.IHistorySync, logger?: ILogger) => {
	...
	const chats: Chat[] = []
	const lidPnMappings: JIDMapping[] = [] // JID-first: stores LID→JID pairs for resolution
	...
	for (const m of item.phoneNumberToLidMappings || []) {
		if (m.lidJid && m.pnJid) {
			lidPnMappings.push({ jid: m.lidJid, pn: m.pnJid })
		}
	}
		...
		case proto.HistorySync.HistorySyncType.ON_DEMAND:
			for (const chat of item.conversations! as Chat[]) {
				contacts.push({
					...
					lid: chat.lidJid || chat.accountLid || undefined, // @deprecated — use id (JID)
				...
				if (isLid && chat.pnJid) {
					lidPnMappings.push({ jid: chatId, pn: chat.pnJid })
				} else if (isPn && chat.lidJid) {
					lidPnMappings.push({ jid: chat.lidJid!, pn: chatId })
					...
					if (pnFromReceipt) {
						lidPnMappings.push({ jid: chatId, pn: pnFromReceipt })
					}
				}
