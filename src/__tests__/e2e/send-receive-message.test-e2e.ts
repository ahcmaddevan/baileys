...
describe('E2E Tests', () => {
	...
	// meLid removed — JID-first mode uses meId for self-messaging
	...
	test('should send and download an image message via LID', async () => {
		console.log(`Testing with self-JID: ${sock.user?.id}`)
		...
		try {
			...
			await sock.sendMessage(sock.user!.id, { // JID-first: send to own phone JID
				image: image,
				caption: caption
			})
