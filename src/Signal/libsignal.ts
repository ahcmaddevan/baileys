...
import type { JIDMapping, LIDMapping, SignalAuthState, SignalKeyStoreWithTransaction } from '../Types'
import type { SignalRepositoryWithJIDStore } from '../Types/Signal'
...
import { JIDMappingStore } from './jid-mapping'
// backwards compat alias
import { JIDMappingStore as LIDMappingStore } from './jid-mapping'
...
export function makeLibSignalRepository(
	...
	const jidMapping = new JIDMappingStore(auth.keys as SignalKeyStoreWithTransaction, logger, pnToJIDFunc || pnToLIDFunc)
	const lidMapping = jidMapping // alias for internal backwards compat
	const storage = signalStorage(auth, jidMapping)
	...
	const repository: SignalRepositoryWithJIDStore = {
		...
		// JID mapping store (replaces LID mapping)
		jidMapping,
		// backwards compat alias
		lidMapping: jidMapping,
...
function signalStorage(
	...
	// Resolve any signal protocol address to its canonical form.
	// LID addresses are transparently resolved to JID addresses.
	const resolveLIDSignalAddress = async (id: string): Promise<string> => {
		if (id.includes('.')) {
			...
			// If it\'s a LID domain, resolve to JID
			if (domainType === WAJIDDomains.LID || domainType === WAJIDDomains.HOSTED_LID) {
				const lidJid = `${user!}${device !== '0' ? `:${device}` : ''}@lid`
				const resolvedJid = await jidMapping.resolveToJID(lidJid)
				if (resolvedJid !== lidJid) {
					return jidToSignalProtocolAddress(resolvedJid).toString()
				}
				return id
			}

			// PN/JID domain — use as-is (JID-first: PN == JID)
		}
