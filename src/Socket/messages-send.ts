...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const getUSyncDevices = async (
		...
		if (result) {
			...
			if (lidResults.length > 0) {
				...
				await signalRepository.jidMapping.storeLIDPNMappings(lidResults.map(a => ({ lid: a.lid as string, pn: a.id })))
	...
	const assertSessions = async (jids: string[], force?: boolean) => {
		...
		if (jidsRequiringFetch.length) {
			...
			const resolvedLidJids = await Promise.all(
				jidsRequiringFetch
					.filter(jid => !!isLidUser(jid) || !!isHostedLidUser(jid))
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroupOrStatus && !isRetryResend) {
				...
				if (!isRetryResend) {
					...
					if (user !== ownUser) {
						...
						const ownUserForAddressing = jidDecode(meId)!.user // JID-first: always PN user
					...
					if (additionalAttributes?.['category'] !== 'peer') {
						...
						const senderIdentity = jidEncode(jidDecode(meId)?.user!, 's.whatsapp.net', undefined)
			...
			if (isRetryResend) {
				...
				const _isParticipantLid = isLidUser(participant!.jid)
