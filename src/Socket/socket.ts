...
import type { JIDMapping, SocketConfig } from '../Types'
...
export const makeSocket = (config: SocketConfig) => {
	...
	const pnFromJIDUSync = async (jids: string[]): Promise<JIDMapping[] | undefined> => {
		...
		return results.list.map(({ lid, id }) => ({ pn: id, jid: lid ? lid as string : id }))
		}
